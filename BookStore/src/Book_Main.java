...
public class Book_Main {
    ...
    {
        try {
            con =DriverManager.getConnection("jdbc:mysql://localhost:3306/bookstore", "root", "Meha@2904");
        }
        catch(Exception e)
        {
            e.printStackTrace();
	...
	public static String isValidPassword()
    {
		...
		System.out.println("(your password should contain minimum 8 characters with an uppercase,lowercase,special characters and numbers)");
	...
	public void cinsert(String str,int q) throws Exception{
		getConection();
		String query="INSERT INTO purchase (book_name,quantity,purchase_time) values ('"+str+"','"+q+"',CURRENT_TIMESTAMP))";
	  	Statement sat=con.createStatement();
	  	int resultset=sat.executeUpdate(query);
